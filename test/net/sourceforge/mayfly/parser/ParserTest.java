...
//          assertEquals("expected non-boolean expression but got 5 = x", e.getMessage());
    ...
    public void testParseAutoIncrement() throws Exception {
        Column column = new Parser("x integer auto_increment primary key")
            .parseColumnDefinition(new CreateTable("foo"));
        assertTrue(column.isSequenceOrAutoIncrement());
        assertTrue(column.isAutoIncrement());
        assertFalse(column.isSequence());
    }
    
    public void testParseSequence() throws Exception {
        Column column = new Parser("x integer generated by default as identity")
            .parseColumnDefinition(new CreateTable("foo"));
        assertTrue(column.isSequenceOrAutoIncrement());
